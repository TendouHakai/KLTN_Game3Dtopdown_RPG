...
void UGlobalCheatManager::ItemAdd(int32 ItemReckey /*= 1*/, int32 ItemCount /*= 1*/, bool IsInBakcpack /*= false*/)
{
	UItemMgr* mgr = GetMgr(UItemMgr);
	if (nullptr == mgr) return;
	mgr->AddItem(ItemReckey, ItemCount, IsInBakcpack);

	// end cheat
	Endcheat();
}

void UGlobalCheatManager::ItemEquipmentAdd(int32 ItemEquipmentReckey /*= 1*/, int32 ItemUpgradeLevel /*= 1*/, bool IsInBakcpack /*= false*/)
{
	UItemMgr* mgr = GetMgr(UItemMgr);
	if (nullptr == mgr) return;
	mgr->AddItemEquipment(ItemEquipmentReckey, ItemUpgradeLevel, IsInBakcpack);
